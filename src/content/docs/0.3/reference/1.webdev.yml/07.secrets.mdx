...
tableOfContents:
  minHeadingLevel: 3
  maxHeadingLevel: 4
slug: 0.3/reference/1webdevyml/07secrets
---
...
This only works together with the attribute `missingMessage` to further provide helpful informations for the user to add the secret. It defines a list of secrets that the file should contain, so that workspace can work correctly. More defailed explainations can be found under [secrets](/0.3/secrets).
